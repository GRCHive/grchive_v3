...
var mysqlDynamicPrivileges = []string{
...
func TestGetUserListing(t *testing.T) {
	...
	for _, version := range []string{
		...
		// 2) Test Dynamic Privileges
		// 3) Test database level privileges
		// 4) Test table level privileges
		// 5) Test column level privileges
		// 6) Test process level privileges
		// 7) Test granting roles to users
		container, db := mysql_utility.SetupMySQLDatabase(t, version, func(db *sqlx.DB) error {
			...
			// Setup test users for testing dynamic privileges.
			for _, priv := range mysqlDynamicPrivileges {
				username := fmt.Sprintf("test_user_%d", userCount)
				usernameWithHost := fmt.Sprintf("%s@%%", username)
			...
			if _, err := tx.Exec(fmt.Sprintf(`
				GRANT '%s', '%s' TO '%s'
			`, parentRole1, parentRole2, childUser)); err != nil {
				tx.Rollback()
				return err
			}
		...
		connector := &EtlMysqlV8ConnectorUser{db: &databases.DB{
			SqlxLike: db,
		}}
		...
		test_utility.CompareUserListing(g, users, refUsers, test_utility.CompareUserListingOptions{
			UsersToIgnore:             []string{"root@%", "root@localhost", "mysql.sys@localhost", "mysql.infoschema@localhost", "mysql.session@localhost"},
